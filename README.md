...
【2022.4.27】继续看八股文 + 基础知识 + 开始[用 rust 刷题](https://leetcode-cn.com/u/sishenhei7/)

- 错误和异常
1.在js里面错误和异常是有区别的，错误是 Error 的一个实例，错误在被创建之后可以被传递给另一个函数或者被抛出；如果错误被抛出，那么它就变成了一个异常。
2.在node.js里面错误处理的几种方法: ```callback(err, data)```回调约定、throw/try/catch、EventEmitter的error事件。
3.如果异常未能被处理，那么与其继续运行，还不如优雅的crash，因为如果继续运行的话你不知道会发生什么事。
4.异步回调会丢失错误栈，很难调试；但是async和await可以捕捉到错误栈。
5.vue里面通过$parent获取父节点来打印组件节点的错误调用栈。
6.vue是怎么捕捉nexttick里面的错误的呢？它会自己把nexttick的回调封装到一个函数中，这个函数在执行用户回调的时候带有错误捕捉函数。然后再timerFunc刷新的时候，flush这个函数队列。（nexttick会丢失组件调用栈）
...
【2022.4.29】继续看八股文 + 基础知识

- 原型链、继承、类

【2022.4.30】在看rust权威指南

- 字符串
1.字符串分为字符串字面量（```&str```）和字符串（```String```）
2.字符串切片是字符串字面量，或者说字符串字面量是字符串切片，字符串切片的数据结构是指向字符串的头指针+长度，所以字符串字面量表示的字符串不在堆里面，而是在二进制数据区域，然后字符串字面量的数据结构其实就是指向这个数据区的头指针+长度，所以字符串字面量就是字符串切片。
3.字符串的解引用也是字符串切片，也是字符串字面量。
4.字符串很复杂，不能通过索引直接索引字符，因为字符串使用utf8储存的，len方法返回的是字节数，索引也是索引的字节，而部分字符是占多个字节的，所以不能使用索引（第xx个字节）来索引字符。另一方面直接索引字符实现看起来像是一个O(1)的操作，但是实际上需要O(n)来实现，所以不能通过所以来直接索引字符。
